...
export const getTimeStringFromNumbers = (days, hours, minutes, seconds) => {
    if (days) return `${plur(days, 'day')}`;
    if (hours) return `${plur(hours, 'hour')}`;
    if (minutes) return `${plur(minutes, 'minute')}`;
    if (seconds) return `${plur(seconds, 'second')}`;
};

export const getFullTimeStringFromNumbers = (days, hours, minutes, seconds) => (
    `${plur(days, 'day')}${plur(hours, 'hour')}${plur(minutes, 'minute')}${plur(seconds, 'second')}`.trim()
);

export const timeLeft = (timeInMs, full = false) => {
    ...
    return full
        ? getFullTimeStringFromNumbers(days, hours, minutes, seconds)
        : getTimeStringFromNumbers(days, hours, minutes, seconds);
}
